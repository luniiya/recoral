...
{#if !scrubbingStore.active && !isNativePlatform() && !selectionStore.active}
	<!-- One shared volume control for pages that show many simultaneous inline
	     ...
	     AudioPlayer's own in-panel slider). Also hidden during multi-select,
	     desktop included: it sits in the same bottom-right corner selecting
	     cards is happening in/near, floating on top and getting in the way,
	     unlike the scrubber/native cases this one isn't about a conflicting
	     control, just physical overlap with what you're doing. -->
	<div
