<script lang="ts">
	...
	interface Props {
		...
		// Bin cards want the backup/sync icon right next to the title instead
		// of grouped with favorite/duration on the right, which they replace
		// with their own content (days-left, speed) via rightSlot.
		backupIconAfterTitle?: boolean;
		rightSlot?: Snippet;
	}

	let { recording, backupIconAfterTitle = false, rightSlot }: Props = $props();
</script>
...
{#snippet backupIcon()}
	{#if isNativePlatform()}
		...
		{:else if recording.syncStatus === 'local-and-synced'}
			<svg viewBox="0 0 512 512" class="size-3.5 shrink-0 text-gray-400" aria-label="On device and backed up">
		...
		{:else}
			<svg viewBox="0 0 512 512" class="size-3.5 shrink-0 text-gray-400" aria-label="Backed up, not on this device">
...
<div class="flex items-baseline justify-between gap-3">
	<span class="flex min-w-0 flex-1 items-center gap-1.5">
		...
		{#if backupIconAfterTitle}{@render backupIcon()}{/if}
	</span>
	{#if rightSlot}
		{@render rightSlot()}
	{:else}
