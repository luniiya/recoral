...
```bash
mkdir recoral && cd recoral # you want to go into the directory where you want to install the container
...
recreates the containers with it. Your recordings and models live in named
Docker volumes, not in the containers themselves, so this doesn't touch your
data. Pin `RECORAL_VERSION` to a specific release tag in `.env` instead of
`latest` if you'd rather upgrade deliberately than automatically track new
releases.
